...
for current_day in range(1, remaining_days + 1):
    ...
    if current_day % 2 == 0:
        ...
        total_spirit += 5
    if current_day % 3 == 0:
        total_cost += quantity_of_decoration * (tree_skirt_set_price + tree_garland_set_price)
        total_spirit += 10 + 3
    if current_day % 5 == 0:
        total_cost += quantity_of_decoration * tree_lights_set_price
        total_spirit += 17
        if current_day % 3 == 0:
            total_spirit += 30
    if current_day % 10 == 0:
        total_spirit -= 20
        total_cost += tree_skirt_set_price + tree_garland_set_price + tree_lights_set_price
